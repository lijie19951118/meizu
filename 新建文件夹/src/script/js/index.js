...
;(function($){
	//轮播图
	var $banner=$('.banner');
	var $bigbox=$('.banner .img-box');
	var $imgbox=$('.banner .img-box div');
	var $btns=$('.banner-btn span');
	var $firstpic=$imgbox.first().clone();
	var $lastpic=$imgbox.last().clone();
	var $divwidth= document.body.clientWidth;
	$bigbox.append($firstpic);
	$bigbox.prepend($lastpic);
	var $lisize=$('.banner .img-box div').size();
	var $num=0;
	$bigbox.css({
		width:$lisize*$divwidth,
		left:-$divwidth
	});
	...
	function tabswitch(){
		...
		$bigbox.animate({
			...
			if(parseInt(bigbox.css('left'))==-divwidth*($btns.size()+1)){
				bigbox.css('left',-divwidth+'px');
				$num=0;
			}
			if(parseInt(bigbox.css('left'))==0){
				bigbox.css('left',-divwidth*$btns.size()+'px');
				$num=$btns.size()-1;
			}
			$bstop=true;
		});
